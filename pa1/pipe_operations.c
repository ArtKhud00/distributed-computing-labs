...
void set_pipe_descriptors(process_content* processContent, int process_count) {
    for(uint8_t from = 0; from < process_count - 1; ++from){
        for(uint8_t to = 1; to < process_count; ++to){
            ...
            //*(readpipe + from * MAX_PROCESS_NUM + to) = file_descriptors[0];
            //*(writepipe + from * MAX_PROCESS_NUM + to) = file_descriptors[1];
            pipe(file_descriptors);
            processContent->read_pipes[to][from] = file_descriptors[0];
            processContent->write_pipes[to][from] = file_descriptors[1];
            //*(readpipe + to * MAX_PROCESS_NUM + from) = file_descriptors[0];
            //*(writepipe + to * MAX_PROCESS_NUM + from) = file_descriptors[1];
            // add logging
        }
    }
}
    ...
    for(uint8_t from = 0; from < process_count; ++from){
        for(uint8_t to = 0; to < process_count; ++to){
            if(to != processContent->this_process && from != processContent->this_process && from != to){
                //close(*(writepipe+from * MAX_PROCESS_NUM + to));
                close(processContent->write_pipes[from][to]);
                //close(*(readpipe+from * MAX_PROCESS_NUM + to));
