...
void handle_requests_with_and_without_cs(process_content *content) {
    ...
    while(1) {
        ...
        if (done_sent == 0 && loop_iteration > MAX_LOOP_COUNT && content->using_mutex) {
            send_DONE_message(content);
            logging_process_done(get_lamport_time_value(), content->this_process);
            done_sent = 1;
            continue;
        ...
        if (loop_iteration <= MAX_LOOP_COUNT && should_send_request(content) && content->using_mutex) {
            request_cs(content);
            continue;
        }

        if (should_release_fork(content) && content->using_mutex) {
            release_cs(content);
            continue;
        }
